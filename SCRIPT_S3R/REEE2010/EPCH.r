...
PCH_1301na<-PCH[PCH$PARAMETRE=="1301" & is.na(PCH$CONTEXTE_PISCICOLE),] #sert uniquement dans le rbinb pour recréer le tableau final
PCH_AUTRE<-PCH[!(PCH$PARAMETRE=="1301"),] #sert uniquement dans le rbinb pour recréer le tableau final
...
# le rang 90 est calculé selon la formule SEQ-EAU : 
#http://www.rade-brest.fr/VIPRR91/Interligo.web.Front/front.aspx?SectionId=106&Controller=ViewPublication&Print=1&PubliId=321
if(nrow(PCHDESC)>0){
	PCHDESC$UN<-1
...
# Selection des stations n'étant pas dans les résultats d'ETATPCH et ayant des données modélisées
	...
	# PEGASE : intégration de PEGASE au résultat non modélisé
		##jointure pour ajouter les données Pégase des stations pour lesquelles on remplace la PCH monitorée par PCH modélisée (PEGASE)
		# PCHFINALEXCEPT_ASSOUP<-merge(PCHFINALEXCEPT_ASSOUP, PEGASE, by="STATION", all.x=TRUE)

		##identification des stations sans état PCH pour importer la PCH modélisée (PEGASE)
		##comme il y a les mêmes noms de colonne entre le merge précedent et celui-ci, R importe automatiquement dans les bonnes colonnes plutot que recréer des colonnes.
		##merge sans by="XXX", remplace le rbind
		# PEGASE_IMPORT<-PEGASE[!(PEGASE$STATION %in% c(PCHFINALEXCEPT_ASSOUP$STATION)),]
		# PCHFINALEXCEPT_ASSOUP<-merge(PCHFINALEXCEPT_ASSOUP, PEGASE_IMPORT, all=TRUE)
		
		# statpeg<-unique(DATAPEGASE$STATION)

		##actualisation des classes d'état PCH avec les classes d'état modélisées
		# PCHFINALEXCEPT_ASSOUP$ETATPCH[!is.na(PCHFINALEXCEPT_ASSOUP$PEGETATPCH)]<-PCHFINALEXCEPT_ASSOUP$PEGETATPCH[!is.na(PCHFINALEXCEPT_ASSOUP$PEGETATPCH)]
		# PCHFINALEXCEPT_ASSOUP$bilano2[!is.na(PCHFINALEXCEPT_ASSOUP$PEGBILANO2)]<-PCHFINALEXCEPT_ASSOUP$PEGBILANO2[!is.na(PCHFINALEXCEPT_ASSOUP$PEGBILANO2)]
		# PCHFINALEXCEPT_ASSOUP$nut[!is.na(PCHFINALEXCEPT_ASSOUP$PEGNUT)]<-PCHFINALEXCEPT_ASSOUP$PEGNUT[!is.na(PCHFINALEXCEPT_ASSOUP$PEGNUT)]
		# PCHFINALEXCEPT_ASSOUP$temp[!is.na(PCHFINALEXCEPT_ASSOUP$PEGTEMP)]<-PCHFINALEXCEPT_ASSOUP$PEGTEMP[!is.na(PCHFINALEXCEPT_ASSOUP$PEGTEMP)]
		# PCHFINALEXCEPT_ASSOUP$acid[!is.na(PCHFINALEXCEPT_ASSOUP$PEGACID)]<-PCHFINALEXCEPT_ASSOUP$PEGACID[!is.na(PCHFINALEXCEPT_ASSOUP$PEGACID)]

		# PCHFINALEXCEPT_ASSOUP$o2[!is.na(PCHFINALEXCEPT_ASSOUP$PEGO2)]<-PCHFINALEXCEPT_ASSOUP$PEGO2[!is.na(PCHFINALEXCEPT_ASSOUP$PEGO2)]
		# PCHFINALEXCEPT_ASSOUP$sato2[!is.na(PCHFINALEXCEPT_ASSOUP$PEGSATO2)]<-PCHFINALEXCEPT_ASSOUP$PEGSATO2[!is.na(PCHFINALEXCEPT_ASSOUP$PEGSATO2)]
		# PCHFINALEXCEPT_ASSOUP$dbo5[!is.na(PCHFINALEXCEPT_ASSOUP$PEGDBO5)]<-PCHFINALEXCEPT_ASSOUP$PEGDBO5[!is.na(PCHFINALEXCEPT_ASSOUP$PEGDBO5)]
		# PCHFINALEXCEPT_ASSOUP$cod[!is.na(PCHFINALEXCEPT_ASSOUP$PEGCOD)]<-PCHFINALEXCEPT_ASSOUP$PEGCOD[!is.na(PCHFINALEXCEPT_ASSOUP$PEGCOD)]
		# PCHFINALEXCEPT_ASSOUP$po43[!is.na(PCHFINALEXCEPT_ASSOUP$PEGPO43)]<-PCHFINALEXCEPT_ASSOUP$PEGPO43[!is.na(PCHFINALEXCEPT_ASSOUP$PEGPO43)]
		# PCHFINALEXCEPT_ASSOUP$phos[!is.na(PCHFINALEXCEPT_ASSOUP$PEGPHOS)]<-PCHFINALEXCEPT_ASSOUP$PEGPHOS[!is.na(PCHFINALEXCEPT_ASSOUP$PEGPHOS)]
		# PCHFINALEXCEPT_ASSOUP$nh4[!is.na(PCHFINALEXCEPT_ASSOUP$PEGNH4)]<-PCHFINALEXCEPT_ASSOUP$PEGNH4[!is.na(PCHFINALEXCEPT_ASSOUP$PEGNH4)]
		# PCHFINALEXCEPT_ASSOUP$no2[!is.na(PCHFINALEXCEPT_ASSOUP$PEGNO2)]<-PCHFINALEXCEPT_ASSOUP$PEGNO2[!is.na(PCHFINALEXCEPT_ASSOUP$PEGNO2)]
		# PCHFINALEXCEPT_ASSOUP$no3[!is.na(PCHFINALEXCEPT_ASSOUP$PEGNO3)]<-PCHFINALEXCEPT_ASSOUP$PEGNO3[!is.na(PCHFINALEXCEPT_ASSOUP$PEGNO3)]
		# PCHFINALEXCEPT_ASSOUP$tempe[!is.na(PCHFINALEXCEPT_ASSOUP$PEGTEMPE)]<-PCHFINALEXCEPT_ASSOUP$PEGTEMPE[!is.na(PCHFINALEXCEPT_ASSOUP$PEGTEMPE)]
		# PCHFINALEXCEPT_ASSOUP$phmin[!is.na(PCHFINALEXCEPT_ASSOUP$PEGPHMIN)]<-PCHFINALEXCEPT_ASSOUP$PEGPHMIN[!is.na(PCHFINALEXCEPT_ASSOUP$PEGPHMIN)]
		# PCHFINALEXCEPT_ASSOUP$phmax[!is.na(PCHFINALEXCEPT_ASSOUP$PEGPHMAX)]<-PCHFINALEXCEPT_ASSOUP$PEGPHMAX[!is.na(PCHFINALEXCEPT_ASSOUP$PEGPHMAX)]
		# PCHFINALEXCEPT_ASSOUP$MODELISE[!is.na(PCHFINALEXCEPT_ASSOUP$PEGETATPCH)]<-"oui"

		# PCHFINALEXCEPT_ASSOUP<-PCHFINALEXCEPT_ASSOUP[,PCHFINALEXCEPT_ASSOUP_NAMES]
		flush.console()
	} else {
	SEEEPEGASE<-"non"
