...
if (SEEECHIM=="oui") {
	...
	# CONSTRUCTION DES TABLEAUX - STATISTIQUE
	##########################################
		RLT_STATCHIMSTATION$NB_STAT_TRAITEES<-rowSums(RLT_STATCHIMSTATION[,2:ncol(RLT_STATCHIMSTATION)]) # les colonnes 2à9 sont NONQUALIFIE BON  MAUVAIS
...
if (SEEECHIM=="oui" & SEEECHIMME=="oui" ) {
	...
	colonnesel<-names(RLT_CHIMETATME) %in%  c("ME", "ETATCHIMME" , "ETATCHIM_SSHAPUBI","METAUX", "PEST" , "POLIND" , "AUTREPOL"  , toupper(PARAGROUPCHIM$PARAGROUPLIBCOURT)  )  
	...
	# CONSTRUCTION DES TABLEAUX - STATISTIQUE
	##########################################
		RLT_STATCHIMME$NB_STAT_TRAITEES<-rowSums(RLT_STATCHIMME[,2:ncol(RLT_STATCHIMME)]) # les colonnes 2à9 sont NONQUALIFIE TRESBON BON MOYEN MEDIOCRE MAUVAIS
