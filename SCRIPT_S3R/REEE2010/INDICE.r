...
# TABLEAU FINAL : mise en forme
	...
	PCHELT_temp<-aggregate(INDICE ~ STATION + ELTQUALITE, data = RLT_PCH , min) # calcul de l'indice déclassant par élément
	...
	if (BASSIN=="AESN") {
		PCHETATINDICE<-aggregate(ETATELT ~ STATION , data = PCHELT_temp , min) # calcul de l'indice de l'état PCH
...
#calcul indice IBD
	...
	BIO_MOY$INDICEBIO[condIBD5]<-round((((BIO_MOY$RESULTAT[condIBD5]-BIO_MOY$INFR[condIBD5])*20)/(BIO_MOY$INFO[condIBD5]-BIO_MOY$INFR[condIBD5]))+0.0)
	BIO_MOY$INDICEBIO[BIO_MOY$PARAGROUP=="5856" & BIO_MOY$INDICEBIO>100]<-100
...
#calcul indice IBG & IBGA
	...
	BIO_MOY$INDICEBIO[condIBG5]<-round((((BIO_MOY$RESULTAT[condIBG5]-BIO_MOY$INFR[condIBG5])*20)/(BIO_MOY$INFO[condIBG5]-BIO_MOY$INFR[condIBG5]))+0.0)
	BIO_MOY$INDICEBIO[BIO_MOY$PARAGROUP %in% c("5910","6951") & BIO_MOY$INDICEBIO>100]<-100
	BIO_MOY<-BIO_MOY[,c(BIO_MOY_NAMES, "INDICEBIO")]
	rm(condIBG1, condIBG2, condIBG3, condIBG4, condIBG5)

#calcul indice IPR
	BIO_MOY<-merge(BIO_MOY,GRILLEIPR,by="PARAGROUP", all.x=TRUE, sort = FALSE)
	...
	# modifié le 30/01/14 pour permettre le paramétrage des valeurs seuils
	...
	BIO_MOY$INDICEBIO[condIPR5]<-round((((BIO_MOY$INFR[condIPR5]-BIO_MOY$RESULTAT[condIPR5])*20)/(BIO_MOY$INFR[condIPR5]-BIO_MOY$INFO[condIPR5]))+0.0)
...
# TABLEAU FINAL : mise en forme
	...
	if (BASSIN=="AESN") {
		TEMP_BIOINDICEETAT<-aggregate(INDICEBIO ~ STATION , data = BIO_MOY , min) # calcul de l'indice de l'état BIO
