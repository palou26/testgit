...
# Calcul des indicateurs BASSINS nécessaires au calcul du niveau de confiance
	...
	# au moins 1 végétal est disponible
		NIVCONF$VEGETAL[!(is.na(NIVCONF$IBD))]<-"oui"
		NIVCONF$VEGETAL[is.na(NIVCONF$IBD)]<-"non"
	...
	# tous les éléments bio sont disponibles
		NIVCONF$BIOALL<-"oui"
		NIVCONF$BIOALL[((is.na(NIVCONF$IBD) | NIVCONF$IBGIBGA=="non dispo" | is.na(NIVCONF$IPR)) & NIVCONF$TYPEME=="MEN") | (is.na(NIVCONF$IBD) & !(NIVCONF$TYPEME=="MEN"))]<-"non"
	...
	# Données sensibles disponibles si risque MACRO
		...
		NIVCONF$DATAMACRO[condRMACRO1 | condRMACRO2]<-"1" # données dispo
		NIVCONF$DATAMACRO[!(condRMACRO1 | condRMACRO2)]<-"2" # données non dispo
...
# Calcul du niveau de confiance
	if (BASSIN %in% c("AEAG", "AERMC", "AERM", "AEAP", "ODE971", "ODE972", "ODE973", "ODE974", "ODE976")) {
		...
		NIVCONF$NIVEAU[condQ10]<-1
		NIVCONF$CAS[condQ10]<-"Q10"
		NIVCONF$NIVEAU[condQ11]<-0
		NIVCONF$CAS[condQ11]<-"Q11"
		rm(condQ1,condQ2,condQ3,condQ4,condQ5,condQ6,condQ7a,condQ7b,condQ8,condQ9a,condQ9b,condQ10,condQ11)	
	}
...
if (SEEEECOLOME=="oui" & CEPE == "PE") {
	...
	if (EXCEPT_MEAFM_ECO == "oui") {
		condmeafm<-EECOLOME$TYPEME %in% c("MEA","MEFM"  )
		EECOLOME$ETATBIO[condmeafm]<-EECOLOME$CHLOA[condmeafm]
		EECOLOME$ETATECOLO[condmeafm]<-pmax(EECOLOME$ETATPCH[condmeafm]  ,  EECOLOME$ETATBIO[condmeafm])
		EECOLOME$RESTRICTION[condmeafm]<-"calcul etateco avec Chloa et PCH uniquement"
