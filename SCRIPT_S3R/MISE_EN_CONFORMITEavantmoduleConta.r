...
# AESN : IBGA est intégrer avec IBG, on retire ce paramètre de la table PARAMETREBIO pour éviter d'avoir les colonnes dans les exports
if (BASSIN =="AESN") {
...
if(MODULE == "REEE2016" | MODULE == "REEE2021"){ # modifié le 10/12/14
...
##GRILLEIBG
if (file.exists(ifelse(MODULE=="REEE2021",paste(ch_param,"GRILLEI2M2",fE.MOD,".csv",sep=""),paste(ch_param,"GRILLEIBG",fE.MOD,".csv",sep=""))) ){
GRILLEIBG<-read.csv2(ifelse(MODULE=="REEE2021",paste(ch_param,"GRILLEI2M2",fE.MOD,".csv",sep=""),paste(ch_param,"GRILLEIBG",fE.MOD,".csv",sep="")))
...
tkmessageBox(title = "Info", message = paste(ch_param,ifelse(MODULE=="REEE2021","GRILLEI2M2","GRILLEIBG"),fE.MOD,".csv n'existe pas.\n L'application va se fermer automatiquement. Veuillez réparer l'erreur et lancer un nouveau traitement.",sep=""), icon = "info", type = "ok")
source(paste(pathS,"msgerror.r",sep="") , echo = TRUE, print.eval = TRUE) #=quit
}
...
if(MODULE == "REEE2016" | MODULE == "REEE2021"){
...
## CONTAMINATION Message d'erreur si pas de fichier parametrage de calcul de contamination
if(CONTA == "oui" &  !file.exists(paste(ch_param,"PARAMETRECONTA",fE.MOD,".csv",sep=""))) {
	tkmessageBox(title = "Info", message = paste(ch_param,"PARAMETRECONTA",fE.MOD,".csv n'existe pas.\n L'application va se fermer automatiquement. Veuillez réparer l'erreur et lancer un nouveau traitement.",sep=""), icon = "info", type = "ok")
	tcl("update")
	source(paste(pathS,"msgerror.r",sep="") , echo = TRUE, print.eval = TRUE) #=quit
} 

if(CONTA == "oui" &  !file.exists(paste(ch_param,"PARAGROUPCONTA",fE.MOD,".csv",sep=""))) {
	tkmessageBox(title = "Info", message = paste(ch_param,"PARAGROUPCONTA",fE.MOD,".csv n'existe pas.\n L'application va se fermer automatiquement. Veuillez réparer l'erreur et lancer un nouveau traitement.",sep=""), icon = "info", type = "ok")
	tcl("update")
	source(paste(pathS,"msgerror.r",sep="") , echo = TRUE, print.eval = TRUE) #=quit
}
}
...
if (file.exists(paste(ch_param,"PARAMETRECHIM",fE.MOD,".csv",sep="")) ){
	if(CONTA != "oui") {PARAMETRECHIM<-read.csv2(paste(ch_param,"PARAMETRECHIM",fE.MOD,".csv",sep=""))} else {PARAMETRECHIM<-read.csv2(paste(ch_param,"PARAMETRECONTA",fE.MOD,".csv",sep=""))}
...
if (file.exists(paste(ch_param,"PARAGROUPCHIM",fE.MOD,".csv",sep="")) ){
	if(CONTA != "oui") {PARAGROUPCHIM<-read.csv2(paste(ch_param,"PARAGROUPCHIM",fE.MOD,".csv",sep=""))} else {PARAGROUPCHIM<-read.csv2(paste(ch_param,"PARAGROUPCONTA",fE.MOD,".csv",sep=""))}
...
if (CEPE == "CE") {
	...
	STATION$EXCEPT_FROID<-tolower(as.character(STATION$EXCEPT_FROID))
	STATION$EXCEPT_CHAUD<-tolower(as.character(STATION$EXCEPT_CHAUD))
	STATION$EXCEPT_ACID<-tolower(as.character(STATION$EXCEPT_ACID))
	STATION$EXCEPT_MO<-tolower(as.character(STATION$EXCEPT_MO))
	STATION$EXCEPT_TOURB<-tolower(as.character(STATION$EXCEPT_TOURB))
...
###############################################
if (SEEEPCH == "oui"  | SEEEPS == "oui" | SEEECHIM == "oui") {
...
###############################
if ((SEEEECOLOME=="oui" | SEEECHIMME=="oui") & CEPE == "CE") {
...
if ((SEEEECOLOME=="oui" | SEEECHIMME=="oui") & CEPE == "PE") {
