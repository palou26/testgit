...
#### Au cas où , il n'y ai pas de d'exception loc , on créé quand même la colonne "ETATPCHssExceptloc"
if (!("ETATPCHssExceptloc" %in% names(PCHFINALEXCEPT_ASSOUP))) {PCHFINALEXCEPT_ASSOUP$ETATPCHssExceptloc <- PCHFINALEXCEPT_ASSOUP$ETATPCH}  


# On calcule à nouveau le bilano2 sans exception
if(!("o2" %in% names(PCHFINALEXCEPT_ASSOUP))) PCHFINALEXCEPT_ASSOUP$o2<-NA
if(!("sato2" %in% names(PCHFINALEXCEPT_ASSOUP))) PCHFINALEXCEPT_ASSOUP$sato2<-NA
if(!("cod" %in% names(PCHFINALEXCEPT_ASSOUP))) PCHFINALEXCEPT_ASSOUP$cod<-NA
if(!("dbo5" %in% names(PCHFINALEXCEPT_ASSOUP))) PCHFINALEXCEPT_ASSOUP$dbo5<-NA

PCHFINALEXCEPT_ASSOUP$bilano2ssexcep<- pmax(PCHFINALEXCEPT_ASSOUP$o2, PCHFINALEXCEPT_ASSOUP$sato2, PCHFINALEXCEPT_ASSOUP$cod, PCHFINALEXCEPT_ASSOUP$dbo5 , na.rm=TRUE)
PCHFINALEXCEPT_ASSOUP$bilano2ssexcep[is.na(PCHFINALEXCEPT_ASSOUP$bilano2ssexcep)]<-PCHFINALEXCEPT_ASSOUP$bilano2[is.na(PCHFINALEXCEPT_ASSOUP$bilano2ssexcep)]

# On fait une jointure PCHFINALEXCEPT_ASSOUP et RLT_BIO
...
if (ASSOUPTBE=="oui") {
	TEMPASSOUP_NIV1<-TEMPASSOUP1[(TEMPASSOUP1$ETATPCHssExceptloc==3 & TEMPASSOUP1$ETATBIO %in% c(1,2) & TEMPASSOUP1$no3 %in% c(1,2,NA)) | (TEMPASSOUP1$ETATPCHssExceptloc==2 & TEMPASSOUP1$ETATBIO==1),] 
	} else {
	TEMPASSOUP_NIV1<-TEMPASSOUP1[TEMPASSOUP1$ETATPCHssExceptloc==3 & TEMPASSOUP1$ETATBIO %in% c(1,2) & TEMPASSOUP1$no3 %in% c(1,2,NA),]
}
...
TEMPASSOUPBE_NIV2c1<-TEMPASSOUP_NIV1[TEMPASSOUP_NIV1$bilano2ssexcep==3 & TEMPASSOUP_NIV1$nut %in% c(1,2, NA) & TEMPASSOUP_NIV1$temp %in% c(1,2, NA) & TEMPASSOUP_NIV1$acid %in% c(1,2, NA),]
TEMPASSOUPBE_NIV2c2<-TEMPASSOUP_NIV1[TEMPASSOUP_NIV1$bilano2ssexcep %in% c(1,2, NA) & TEMPASSOUP_NIV1$nut==3 & TEMPASSOUP_NIV1$temp %in% c(1,2, NA) & TEMPASSOUP_NIV1$acid %in% c(1,2, NA),]
...
TEMPASSOUPTBE_NIV2c1<-TEMPASSOUP_NIV1[TEMPASSOUP_NIV1$bilano2ssexcep==2 & TEMPASSOUP_NIV1$nut %in% c(1, NA) & TEMPASSOUP_NIV1$temp %in% c(1, NA) & TEMPASSOUP_NIV1$acid %in% c(1, NA),]
TEMPASSOUPTBE_NIV2c2<-TEMPASSOUP_NIV1[TEMPASSOUP_NIV1$bilano2ssexcep %in% c(1, NA) & TEMPASSOUP_NIV1$nut==2 & TEMPASSOUP_NIV1$temp %in% c(1, NA) & TEMPASSOUP_NIV1$acid %in% c(1, NA),]
...
PCHFINALEXCEPT_ASSOUP$nbdeclassbilano2<-c(lapply(split(PCHFINALEXCEPT_ASSOUP[,c("STATION","ETATPCHssExceptloc",PARAMETREPCH$NOM[PARAMETREPCH$ELTQUALITE == "bilano2"]  )], PCHFINALEXCEPT_ASSOUP$STATION), 
function(x)  length(as.numeric(x[3:ncol(x)])[!is.na(as.numeric(x[3:ncol(x)])) & as.numeric(x[3:ncol(x)]) == as.numeric(x[2])])), recursive=T)

PCHFINALEXCEPT_ASSOUP$nbdeclassnut<-c(lapply(split(PCHFINALEXCEPT_ASSOUP[,c("STATION","ETATPCHssExceptloc",PARAMETREPCH$NOM[PARAMETREPCH$ELTQUALITE == "nut"]  )], PCHFINALEXCEPT_ASSOUP$STATION), 
function(x)  length(as.numeric(x[3:ncol(x)])[!is.na(as.numeric(x[3:ncol(x)])) & as.numeric(x[3:ncol(x)]) == as.numeric(x[2])])), recursive=T)

PCHFINALEXCEPT_ASSOUP$nbdeclasstemp<-c(lapply(split(PCHFINALEXCEPT_ASSOUP[,c("STATION","ETATPCHssExceptloc",PARAMETREPCH$NOM[PARAMETREPCH$ELTQUALITE == "temp"]  )], PCHFINALEXCEPT_ASSOUP$STATION), 
function(x)  length(as.numeric(x[3:ncol(x)])[!is.na(as.numeric(x[3:ncol(x)])) & as.numeric(x[3:ncol(x)]) == as.numeric(x[2])])), recursive=T)

PCHFINALEXCEPT_ASSOUP$nbdeclassacid<-c(lapply(split(PCHFINALEXCEPT_ASSOUP[,c("STATION","ETATPCHssExceptloc",PARAMETREPCH$NOM[PARAMETREPCH$ELTQUALITE == "acid"]  )], PCHFINALEXCEPT_ASSOUP$STATION), 
function(x)  length(as.numeric(x[3:ncol(x)])[!is.na(as.numeric(x[3:ncol(x)])) & as.numeric(x[3:ncol(x)]) == as.numeric(x[2])])), recursive=T)
gc() ## compacte R
...
TEMPASSOUP_NIV3c5<-TEMPASSOUP_NIV2[TEMPASSOUP_NIV2$nbdeclassbilano2==2 & (TEMPASSOUP_NIV2$bilano2ssexcep==TEMPASSOUP_NIV2$o2) & (TEMPASSOUP_NIV2$bilano2ssexcep==TEMPASSOUP_NIV2$sato2) & TEMPASSOUP_NIV2$nbdeclassnut %in% c(0, NA) & TEMPASSOUP_NIV2$nbdeclasstemp %in% c(0, NA) & TEMPASSOUP_NIV2$nbdeclassacid %in% c(0, NA),]
...
TEMPASSOUPFINAL<-TEMPASSOUP_NIV3[,c("STATION", "ETATPCHssExceptloc", "bilano2ssexcep", "nut", "temp", "acid")]
names(TEMPASSOUPFINAL)[2]<-"ETATPCHssExceptlocassoup"
...
condBO2<-TEMPASSOUPFINAL$bilano2assoup==TEMPASSOUPFINAL$ETATPCHssExceptlocassoup & !is.na(TEMPASSOUPFINAL$bilano2assoup)
condNUT<-TEMPASSOUPFINAL$nutassoup==TEMPASSOUPFINAL$ETATPCHssExceptlocassoup & !is.na(TEMPASSOUPFINAL$nutassoup)
condTEMP<-TEMPASSOUPFINAL$tempassoup==TEMPASSOUPFINAL$ETATPCHssExceptlocassoup & !is.na(TEMPASSOUPFINAL$tempassoup)
condACID<-TEMPASSOUPFINAL$acidassoup==TEMPASSOUPFINAL$ETATPCHssExceptlocassoup & !is.na(TEMPASSOUPFINAL$acidassoup)
...
TEMPASSOUPFINAL$ETATPCHssExceptlocassoup<-TEMPASSOUPFINAL$ETATPCHssExceptlocassoup-1
...
gc()
PCHFINALEXCEPT_ASSOUP$ETATPCHssExceptloc[!is.na(PCHFINALEXCEPT_ASSOUP$ETATPCHssExceptlocassoup)]<-PCHFINALEXCEPT_ASSOUP$ETATPCHssExceptlocassoup[!is.na(PCHFINALEXCEPT_ASSOUP$ETATPCHssExceptlocassoup)]
...
PCHFINALEXCEPT_ASSOUP$ASSOUPLI[!is.na(PCHFINALEXCEPT_ASSOUP$ETATPCHssExceptlocassoup)]<-"oui"

rm(TEMPASSOUP1, TEMPASSOUP_NIV1,TEMPASSOUP_NIV2,TEMPASSOUP_NIV3)
gc() ## compacte R

# 30/05/2018 : On applique l'assouplissement de ETATPCHssExceptloc  à ETATPCH

cond<-PCHFINALEXCEPT_ASSOUP$ETATPCH > PCHFINALEXCEPT_ASSOUP$ETATPCHssExceptloc & !is.na(PCHFINALEXCEPT_ASSOUP$ASSOUPLI) & PCHFINALEXCEPT_ASSOUP$ASSOUPLI == "oui"
PCHFINALEXCEPT_ASSOUP$ETATPCH[cond]<-PCHFINALEXCEPT_ASSOUP$ETATPCHssExceptloc[cond]
...
FCTEECOLO<- function (VARECOLOHPS=EECOLO$ETATECOLOHPS, VARPS=EECOLO$ETATPS) {
	...
	#algo de calcul
	VARECOLO<-pmax(VARECOLOHPS,VARPS, na.rm=TRUE)
	
	VARECOLO[is.na(VARECOLOHPS)   ]<-NA
	VARECOLO[VARECOLOHPS==0]<-0
	VARECOLO[VARECOLOHPS %in% c(0,1,2) & VARPS==3 & !is.na(VARECOLOHPS) & !is.na(VARPS)]<-3
	VARECOLO[is.na(VARECOLOHPS) & VARPS==3  & !is.na(VARPS)]<-3
																			 
	VARECOLO[VARECOLOHPS == 1 & (is.na(VARPS) | VARPS==0)]<-2
...
# Non prise en compte des Exceptions loclaes en COD et SATO2
if (EXCEPTLOC == "oui") {

	EECOLO$ETATECOLOHPS_ssExceptloc<-FCTEECOLOHPS_AESN(VARBIO=EECOLO$ETATBIO, VARPCH=EECOLO$ETATPCHssExceptloc)
...
gc()
	...
	if (SEEEPS=="oui" & BASSIN!="AESN" ) {
		EECOLO<-merge(EECOLO, RLT_POLSPE[,c("STATION", ETATPS_NAMES)], by="STATION", all.x=TRUE)
		EECOLO$ETATECOLO<-FCTEECOLO(VARECOLOHPS=EECOLO$ETATECOLOHPS, VARPS=EECOLO$ETATPS)
		EECOLO$ETATECOLO_ssExceptloc<-FCTEECOLO(VARECOLOHPS=EECOLO$ETATECOLOHPS_ssExceptloc, VARPS=EECOLO$ETATPS)
	} else if (SEEEPS=="oui" & BASSIN=="AESN" ) {
		EECOLO<-merge(EECOLO, RLT_POLSPE[,c("STATION", ETATPS_NAMES)], by="STATION", all.x=TRUE)
		EECOLO$ETATECOLO<-FCTEECOLO_AESN(VARECOLOHPS=EECOLO$ETATECOLOHPS, VARPS=EECOLO$ETATPS)
		EECOLO$ETATECOLO_ssExceptloc<-FCTEECOLO_AESN(VARECOLOHPS=EECOLO$ETATECOLOHPS_ssExceptloc, VARPS=EECOLO$ETATPS)

	} else {
		EECOLO$ETATECOLO<-EECOLO$ETATECOLOHPS
		EECOLO$ETATECOLO_ssExceptloc<-EECOLO$ETATECOLOHPS_ssExceptloc
	}
...
if (EXCEPTLOC == "oui") {  # Ajout des noms de colonnes pour Exceptions locales
	ETATECOLO_NAMES<-c(ETATECOLO_NAMES,"ETATECOLO_ssExceptloc")
	ETATECOLOHPS_NAMES<-c(ETATECOLOHPS_NAMES,"ETATECOLOHPS_ssExceptloc")
}
...
if (SEEEECOLOME=="oui" & CEPE == "CE") {
	TEMPEECOLOME<-merge(EECOLO[,c("STATION",  ETATPCH_NAMES, "ETATBIO", "IBD", ETATPS_NAMES, ETATECOLO_NAMES, "ASSOUPLI")], STATION[,c("STATION", "EUCD", "REPRESENTATIVE")], by="STATION", all.x=TRUE)
...
## Préparation des données pour calcul l'état ecolo en tenant compte du cas MEFM/MEA
	...
	if (EXCEPTLOC == "oui") { 
	TEMPEECOLOME$ETATPCHMEssExceptloc<-TEMPEECOLOME$ETATPCHssExceptloc #data identique entre analyse à la station et ME, mais c'est pour etre cohérent avec la bio
	}
...
## Etat ECOLO hors polluants spécifiques (appel de la fonction)
	...
	if (EXCEPTLOC == "oui") {
		if (BASSIN=="AESN") {
			TEMPEECOLOME$ETATECOLOHPSMEssExceptloc<-FCTEECOLOHPS_AESN(VARBIO=TEMPEECOLOME$ETATBIOME, VARPCH=TEMPEECOLOME$ETATPCHMEssExceptloc)
		} else {
			TEMPEECOLOME$ETATECOLOHPSMEssExceptloc<-FCTEECOLOHPS(VARBIO=TEMPEECOLOME$ETATBIOME, VARPCH=TEMPEECOLOME$ETATPCHMEssExceptloc)
		}	
	}
...
## Etat ECOLO avec polluants spécifiques (appel de la fonction)
	...
	if (EXCEPTLOC == "oui") {
		if (SEEEPS=="oui" & BASSIN!="AESN" ) {
			TEMPEECOLOME$ETATECOLOMEssExceptloc<-FCTEECOLO(VARECOLOHPS=TEMPEECOLOME$ETATECOLOHPSMEssExceptloc, VARPS=TEMPEECOLOME$ETATPSME)
		} else if (SEEEPS=="oui" & BASSIN=="AESN" ) {
			TEMPEECOLOME$ETATECOLOMEssExceptloc<-FCTEECOLO_AESN(VARECOLOHPS=TEMPEECOLOME$ETATECOLOHPSMEssExceptloc, VARPS=TEMPEECOLOME$ETATPSME)
		} else {
			TEMPEECOLOME$ETATECOLOMEssExceptloc<-TEMPEECOLOME$ETATECOLOHPSMEssExceptloc
		}
	}	
...
## Mise en forme tableau final
	...
	TEMPEECOLOME_NAMES<-names(TEMPEECOLOME[,c("STATION", "ETATBIOME", ETATPCHME_NAMES, ETATPSME_NAMES, "ETATECOLOHPSME", "TYPEME")])
	if (EXCEPTLOC == "oui") {TEMPEECOLOME_NAMES<-names(TEMPEECOLOME[,c("STATION", "ETATECOLOMEssExceptloc","ETATBIOME", ETATPCHME_NAMES, ETATPSME_NAMES, "ETATECOLOHPSME", "TYPEME")])}
