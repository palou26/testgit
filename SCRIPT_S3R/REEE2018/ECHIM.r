...
if ( SUPPR_LQ_ABERRANTE_CHIM == "oui" ) {
	...
	if( nrow(PBLQCHIMABERRANTE)>0) {
		...
		MSGBOX <- tkmessageBox(title = "Info", message = paste("Les données brutes non quantifiées avec LQ aberrantes (strictement > LQSEUIL) ont été retirées \n du calcul de l'état chimique et exportées dans \n",CSV,sep=""), icon = "info", type = "ok")
	}
...
cond1<-DATACHIM$LQ == 0 & DATACHIM$RESULTAT >= 0 & DATACHIM$REMARQUE %in% c(2,7,10)
DATACHIM$LQ[cond1]<-DATACHIM$RESULTAT[cond1]

cond3<-DATACHIM$LQ == 0 & DATACHIM$RESULTAT == 0 & DATACHIM$REMARQUE == 1

	if( nrow(DATACHIM[cond3,])>0) {
		CSV<-paste(CH_ERREUR,"CHIM_LQ_ZERO_QUANTIFIEE_",g,"_",SEEE_DEBformat,".csv",sep="")
		write.csv2(DATACHIM[cond3,],CSV)
		DATACHIM<-DATACHIM[!cond3,]
		MSGBOX <- tkmessageBox(title = "Info", message = paste("Les", nrow(DATACHIM[cond3,])  ," analyses avec 'LQ= 0 et CodeRemarque = 1 et Résultat = 0' \n ont été retirées  du calcul de l'état chimique et exportées dans \n",CSV,sep=""), icon = "info", type = "ok")
	}
...
DATACHIM_FREQ<-aggregate(RESULTAT ~ STATION + PARAGROUP + ANNEE, data = DATACHIM , length) # les données sont déjà filtrées sur l'année, on le prend dans aggregate uniquement pour l'avoir ensuite dans IDAN
...
if ( LQSUPNQEMACHIM == "oui" ) {
	...
	if( nrow(PBLQCHIM)>0) {
		...
		MSGBOX <- tkmessageBox(title = "Info", message = paste("Les données brutes non quantifiées avec LQ > NQEMA ont été retirées \n du calcul de l'état chimique et exportées dans \n",CSV,sep=""), icon = "info", type = "ok")
...
#### METTRE TOUT EN RLT_CHIMSTATIONMAJUSCULE
