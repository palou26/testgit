...
if (EXCEPTLOC == "non") {names(PCH_STATION)[2]<-"ETATPCH"} else {names(PCH_STATION)[2]<-"ETATPCHssExceptloc"} #renomme la colonne ETATELT sur laquelle il y a eu le max car correspond à l'état de la station

# traitement des exception locales en COD et O2/SATO2
if (EXCEPTLOC == "oui") {
	RLT_PCH_EXCEPT<-RLT_PCH
	RLT_PCH_EXCEPT$CLASSEPCH[RLT_PCH_EXCEPT$EXCEPTLOC_COD == "oui" & RLT_PCH_EXCEPT$PARAMETRE == 1841 ]<-NA
	RLT_PCH_EXCEPT$CLASSEPCH[RLT_PCH_EXCEPT$EXCEPTLOC_O2SATO2 == "oui" & RLT_PCH_EXCEPT$PARAMETRE == 1311 ]<-NA
	RLT_PCH_EXCEPT$CLASSEPCH[RLT_PCH_EXCEPT$EXCEPTLOC_O2SATO2 == "oui" & RLT_PCH_EXCEPT$PARAMETRE == 1312 ]<-NA

	PCH_ELTQUALITE_avecEXCEPT<-merge(RLT_PCH_EXCEPT,PARAMETREPCH[,c("PARAMETRE","ELTQUALITE")],by="PARAMETRE")
	PCH_ELTQUALITE_avecEXCEPT<-aggregate(CLASSEPCH ~ STATION + ELTQUALITE, data = PCH_ELTQUALITE_avecEXCEPT , max, na.rm = TRUE)
	PCH_ELTQUALITE_avecEXCEPT<-PCH_ELTQUALITE_avecEXCEPT[order(PCH_ELTQUALITE_avecEXCEPT$STATION, PCH_ELTQUALITE_avecEXCEPT$ELTQUALITE),]
	names(PCH_ELTQUALITE_avecEXCEPT)[3]<-"ETATELT" #renomme la colonne CLASSEPCH sur laquelle il y a eu le max car correspond à l'état de element qualité

	PCH_STATION_avecEXCEPT<-aggregate(ETATELT ~ STATION, data = PCH_ELTQUALITE_avecEXCEPT , max, na.rm=TRUE)
	PCH_STATION_avecEXCEPT<-PCH_STATION_avecEXCEPT[order(PCH_STATION_avecEXCEPT$STATION),]
	names(PCH_STATION_avecEXCEPT)[2]<-"ETATPCH"
	
}

if (EXCEPTLOC == "oui") {
	RLT_PCHFINAL<-merge(PCH_STATION_avecEXCEPT,PCH_STATION, by = "STATION", all = TRUE)
} else {
...
flush.console()
gc() ## compacte R
} else { # Si on traite les exception locales
	for (i in  1:nrow(TABLEELT)) {
		TEMPELT<-PCH_ELTQUALITE_avecEXCEPT[PCH_ELTQUALITE_avecEXCEPT$ELTQUALITE==TABLEELT$ELTQUALITE[i],c("STATION", "ETATELT")]
		names(TEMPELT)[2]<-TABLEELT$ELTQUALITE[i]
		RLT_PCHFINAL<-merge(RLT_PCHFINAL,TEMPELT,by="STATION", all.x=TRUE)
		rm(TEMPELT)
	}
...
if (EXCEPTLOC == "oui") {names(PCHEXCEPT_STATION)[2]<-"ETATPCHssExceptloc"} else {names(PCHEXCEPT_STATION)[2]<-"ETATPCH"} #renomme la colonne ETATELT sur laquelle il y a eu le max car correspond à l'état de la station
gc() ## compacte R


# traitement des exception locales en COD et O2/SATO2
if (EXCEPTLOC == "oui") {
	RLT_PCH_EXCEPT<-PCHEXCEPT_R90
	RLT_PCH_EXCEPT$CLASSEPCH[RLT_PCH_EXCEPT$EXCEPTLOC_COD == "oui" & RLT_PCH_EXCEPT$PARAMETRE == 1841 & RLT_PCH_EXCEPT$CLASSEPCH>1 ]<-NA
	RLT_PCH_EXCEPT$CLASSEPCH[RLT_PCH_EXCEPT$EXCEPTLOC_O2SATO2 == "oui" & RLT_PCH_EXCEPT$PARAMETRE == 1311 & RLT_PCH_EXCEPT$CLASSEPCH>1]<-NA
	RLT_PCH_EXCEPT$CLASSEPCH[RLT_PCH_EXCEPT$EXCEPTLOC_O2SATO2 == "oui" & RLT_PCH_EXCEPT$PARAMETRE == 1312 & RLT_PCH_EXCEPT$CLASSEPCH>1]<-NA
...
if (EXCEPTLOC == "oui") {
	RLT_PCHFINALEXCEPT<-merge(PCH_STATION_avecEXCEPT,PCHEXCEPT_STATION, by = "STATION", all = TRUE)
} else {
