...
if (SEEEPS=="oui") {
	...
	if(exists("POLSPEFREQQUANTI")) {  ##Ajout 20/04/2018 : calcul de frequence de mesures quantifiées
	
		PSFREQQUANTI<-data.frame(table(POLSPEFREQQUANTI$STATION,POLSPEFREQQUANTI$PARAMETRE))
	names(PSFREQQUANTI)[1]<-"STATION"
	names(PSFREQQUANTI)[2]<-"PARAMETRE"
	
	RLT_POLSPEFREQQUANTI<-data.frame(RLT_POLSPE[,c("STATION")])
	names(RLT_POLSPEFREQQUANTI)[1]<-"STATION"
		# mise en forme tableau pour la fréquence
		for (i in  1:nrow(PARAMETREPOLSPE)  ) {
			TEMPP<-PSFREQQUANTI[PSFREQQUANTI$PARAMETRE==PARAMETREPOLSPE$PARAMETRE[i],c("STATION", "Freq")]
			names(TEMPP)[2]<-paste("fq",toupper(PARAMETREPOLSPE$PARAMETRELIB[i]), sep="")
			RLT_POLSPEFREQQUANTI<-merge(RLT_POLSPEFREQQUANTI,TEMPP,by="STATION", all.x=TRUE)
			rm(TEMPP)
		}
