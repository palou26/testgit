...
if (FREQOKCHIM == "oui") {
	...
	if ( LQSUPNQEMACHIM == "oui" ) {
		...
		#CHIMMA<-merge(CHIMMA,PBLQCHIMFREQ[,c("ID","FREQPBLQ")],by="ID",all=TRUE)
...
condCAS1<-CHIMMA$CASCHIMMA != "0"   &  (is.na(CHIMMA$LQMAX) | CHIMMA$LQMAX <= CHIMMA$NQEMA) & CHIMMA$INDIVIDUELLE == "oui"
	# Cas 1A
	condCAS1a<-condCAS1 & CHIMMA$MOY < CHIMMA$LQMAX &  !is.na(CHIMMA$LQMAX)
	CHIMMA$CLASSEMA[condCAS1a]<-1  
	CHIMMA$CASCHIMMA[condCAS1a ]<-"1A"
	# Cas 1B
	condCAS1b<-condCAS1 & CHIMMA$CASCHIMMA != "1A" & CHIMMA$MOY <= CHIMMA$NQEMA 
	CHIMMA$CLASSEMA[condCAS1b]<-1  
	CHIMMA$CASCHIMMA[condCAS1b ]<-"1B"
	# Cas 1B
	condCAS1c<-condCAS1 & CHIMMA$MOY > CHIMMA$NQEMA 
	CHIMMA$CLASSEMA[condCAS1c]<-2  
	CHIMMA$CASCHIMMA[condCAS1c ]<-"1C"
	
###CAS 2 -  la MOYENNE > NQE 
condCAS2<-CHIMMA$CASCHIMMA != "0" &  !is.na(CHIMMA$LQMAX) & CHIMMA$LQMAX > CHIMMA$NQEMA & CHIMMA$INDIVIDUELLE == "oui"
	# Cas 2A
...
condCAS3<-CHIMMA$CASCHIMMA != "0" & CHIMMA$INDIVIDUELLE == "non"
	# Cas 2A
