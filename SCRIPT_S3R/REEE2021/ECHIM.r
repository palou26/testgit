...
if ( SUPPR_LQ_ABERRANTE_CHIM == "oui" ) {
	if (LQ_NONDISPO =="oui") { # si LQ non dispo on prend les valeurs du champ RESULTAT lorsque non quantifié --> car c'est la LQ
		PBLQCHIMABERRANTE<-DATACHIM[DATACHIM$RESULTAT>DATACHIM$LQSEUIL & DATACHIM$QUANTIFIE=="0" & DATACHIM$LQSEUIL != -99 ,]
		DATACHIM<-DATACHIM[!(DATACHIM$RESULTAT>DATACHIM$LQSEUIL & DATACHIM$QUANTIFIE=="0") | DATACHIM$LQSEUIL == -99 ,]
	} else {
		PBLQCHIMABERRANTE<-DATACHIM[DATACHIM$LQ > DATACHIM$LQSEUIL & DATACHIM$LQSEUIL != -99 ,]
		DATACHIM<-DATACHIM[DATACHIM$LQ <= DATACHIM$LQSEUIL | DATACHIM$LQSEUIL == -99  ,]
	}
...
condCAS2<-!( CHIMMA$CASCHIMMA  %in% c("0","0+","0++","0+++","0*","0**","0***")) &  !is.na(CHIMMA$LQMAX) & CHIMMA$LQMAX > CHIMMA$NQEMA & CHIMMA$INDIVIDUELLE == "oui"
	# Cas 2A
...
condCAS3<-!( CHIMMA$CASCHIMMA  %in% c("0","0+","0++","0+++","0*","0**","0***")) & CHIMMA$INDIVIDUELLE == "non"
	# Cas 2A
